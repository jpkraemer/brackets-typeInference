...
define(function (require, exports, module) {
	...
	function getTestCasesCallingFunction (functionIdentifier) {
		...
		_.forOwn(lastTestResults, function (resultArray, suiteName) {
			...
			callingTestResults[suiteName] = filteredResults;
		});

		var callingTestCases = []; 
		_.forOwn(callingTestResults, function (resultArray, suiteName) {
			...
			for (var i = 0; i < resultArray.length; i++) {
				...
				if (result.calledFunctions.length > 0) {
					...
					if (filename.indexOf(autogeneratedPrefix) === 0) {
						var pathToOriginalFile = suiteName.split("-").slice(0, -3).join("-");
					...
					if (testCaseCollection !== undefined) {
						callingTestCases.push(testCaseCollection.getTestCaseForSuiteTitleAndTestCaseId(suiteName, result.id));
					}
				}
			}
		});
